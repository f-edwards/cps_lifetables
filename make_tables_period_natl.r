...
afcars_nat_year<-afcars %>%
  ...
  left_join(pop_nat)  %>% 
  mutate(year = ifelse(year>=2004 & year<=2011, 2004, 2012)) %>% 
  group_by(.imp, year, age, race_ethn) %>% 
  summarise(first_fc = sum(first_fc), pop = sum(pop))
...
write_csv(tables_comb, "./vis/fc_lifetable_period.csv")
...
write_csv(tables_comb, "./vis/malt_lifetable_period.csv")
